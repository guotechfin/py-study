...
class CustomerList(web_handler.WebHandler):
    def prepare(self):
        self.resp_json = ""
        self.ret = 0
    ...
    def response(self):
        ...
        resp_dict['customer_list'] = utility.to_obj(self.customer_list)
    ...
    @utility.check_right(conf.AUTHORIZATION[8][0][0][0])
    def get(self):
        self.customer_list, self.total = (
                customer_obj.search_customer_list(
                    self._db_session,
                    start=self.start,
                    end=self.end))
