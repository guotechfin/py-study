...
class CustomerDel(web_handler.WebHandler):
    def prepare(self):
        self.resp_json = ""
        self.ret = 0
        self.msg = error_msg.SUCCESS
        self.customer_id = self.get_argument("id", -1)
    ...
    @utility.check_right(conf.AUTHORIZATION[8][0][3][0])
    def get(self):
        try:
            customer_obj.del_customer(
                    self._db_session,
                    customer_id=self.customer_id
                    )
            self._db_session.commit()
        except Exception as e:
            mylog.logger.error("del_customer error %s" % e)
            self.ret = 1
