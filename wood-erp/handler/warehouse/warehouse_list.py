...
class WarehouseList(web_handler.WebHandler):
    def prepare(self):
        self.resp_json = ""
        self.ret = 0
    ...
    def response(self):
        ...
        resp_dict['warehouse_list'] = utility.to_obj(self.warehouse_list)
    ...
    @utility.check_right(conf.AUTHORIZATION[6][0][0][0])
    def get(self):
        self.warehouse_list, self.total = (
                warehouse_obj.search_warehouse_list(
                    self._db_session,
                    start=self.start,
                    end=self.end))
