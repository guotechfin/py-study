...
def login_requested(func):
    def _login_requested(*args, **kwargs):
        ...
        if check_token(self._user_id, self._token_id):
            ...
            self.ret, self.msg = error_msg.TOKEN_TIMEOUT_ERROR
...
def to_obj(orm_obj, need_fields=[],
        without_fields=[], datetime_format="%Y-%m-%d"):
    if orm_obj == None:
        return None
    elif isinstance(orm_obj, list):
        obj_list = list()
        for i in orm_obj:
            obj_list.append(to_obj(i, need_fields, without_fields, datetime_format))
        return obj_list
        ...
        for field in [x for x in dir(orm_obj)
            ...
            if need_fields and field not in need_fields:
                continue
